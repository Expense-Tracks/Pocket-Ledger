      ...
      <div className="space-y-2">
        {transactions.map(t => {
          ...
          return (
            <div
              key={t.id}
              className="flex items-center gap-3 rounded-xl bg-card p-3 transition-all hover:bg-accent cursor-pointer"
              onClick={() => setEditingTransaction(t)}
              role="button"
              tabIndex={0}
              onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setEditingTransaction(t); } }}
            >
